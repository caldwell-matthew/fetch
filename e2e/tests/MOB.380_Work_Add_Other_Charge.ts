// Generated from Mobile/dd_tests_mobile/MOB.380_Work_Add_Other_Charge.json by to_playwright.py — do not edit by hand yet.
...
export async function mob380(page: Page): Promise<void> {
  try {
    ...
    // Open the Other tab
    await el(page, `//*[@role="tab"][contains(normalize-space(.), "Other")]`).click({ timeout: DEFAULT_TIMEOUT });
    // Let the Other cards render
    await wait(page, 2);
    // BEFORE: count the other charge cards already here (the server-proof baseline)
...
if (!p) return false;
const needles = ['Other Charge Types', 'Charge created on'];
...
return true;`, 30000);
    ...
    // Focus the other charge type lookup
    await el(page, `//*[@id="otherChargeId"]`).click({ timeout: DEFAULT_TIMEOUT });
    // Type into the other charge type lookup to load options
    await el(page, `//*[@id="otherChargeId"]`).fill(`Other Charge Types`, { timeout: DEFAULT_TIMEOUT });
    // Wait for other charge type options
    await wait(page, 2);
    // Pick the Other Charge Types other charge type option
    await el(page, `//*[@role="option"][contains(normalize-space(.), "Other Charge Types")]`).click({ timeout: DEFAULT_TIMEOUT });
    ...
    await wait(page, 2);
    // ⭐ SERVER PROOF: after a RELOAD there is exactly ONE more other charge card than before
    await assertFromJavascript(page, `const tabEl = document.querySelector('[role="tab"][aria-selected="true"], [role="tab"][data-active]');
const byId = tabEl && tabEl.getAttribute('aria-controls')
  ? document.getElementById(tabEl.getAttribute('aria-controls')) : null;
const p = byId || [...document.querySelectorAll('[role="tabpanel"]')]
