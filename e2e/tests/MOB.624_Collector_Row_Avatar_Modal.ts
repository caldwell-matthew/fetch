...
import { DEFAULT_TIMEOUT, Soft, assertElementPresent, assertFromJavascript, el, optional, wait } from '../support/dd';
...
export async function mob624(page: Page): Promise<void> {
  ...
  try {
    ...
    await soft.run("FIXTURE GUARD: its badge (image/video attachment count) is not 0 \u2014 MOB.623 residue", async () => {
...
return !!b && (b.textContent || '').trim() !== '0';`, 30000);
    ...
    // BASELINE: the row is collapsed and no modal is open
    await assertFromJavascript(page, `const items = [...document.querySelectorAll('.mantine-Accordion-item')];
const it = items.find(i => {
  const c = i.querySelector('.mantine-Accordion-control');
  return c && (c.textContent || '').includes('DD SYNTHETIC MOBILE');
...
return c.getAttribute('aria-expanded') === 'false' && !document.querySelector('.mantine-Modal-content');`, 30000);
    // Click the row's avatar
    await el(page, `(//*[contains(concat(" ", normalize-space(@class), " "), " mantine-Accordion-item ")][.//*[contains(concat(" ", normalize-space(@class), " "), " mantine-Accordion-control ")][contains(., "DD SYNTHETIC MOBILE")]])[1]//*[contains(concat(" ", normalize-space(@class), " "), " mantine-Indicator-root ")]`).click({ timeout: 30000 });
    // Let the fullscreen modal mount
    await wait(page, 3);
    // ⭐ The modal opened, names the asset, and offers `Done` — and the avatar's click did NOT expand the accordion (stopPropagation)
...
return true;`, 30000);
    ...
    await wait(page, 3);
    // ⭐ PHOTOS: a carousel with a slide, `Add Photo`, and NO `Add File`
    await assertFromJavascript(page, `const m = document.querySelector('.mantine-Modal-content');
if (!m) return false;
const t = [...m.querySelectorAll('button')].map(b => (b.textContent || '').trim());
...
return true;`, 30000);
    ...
    await wait(page, 3);
    // ⭐ DOCS: `Add File`, NO `Add Photo`, and NO carousel — the biconditional closes
    await assertFromJavascript(page, `const m = document.querySelector('.mantine-Modal-content');
if (!m) return false;
const t = [...m.querySelectorAll('button')].map(b => (b.textContent || '').trim());
return !m.querySelector('[class*="mantine-Carousel"]') && t.includes('Add File') && !t.includes('Add Photo');`, 30000);
    ...
    // Close the modal with its own `Done`
    ...
    await optional("SENTINEL (bugs \u00a735): the row is EXPANDED \u2014 clicks inside the modal toggled the accordion behind it", async () => {
      await assertFromJavascript(page, `const items = [...document.querySelectorAll('.mantine-Accordion-item')];
const it = items.find(i => {
...
return c.getAttribute('aria-expanded') === 'true';`, DEFAULT_TIMEOUT);
    ...
    // RESTORE: collapse the row (§35 left it open)
    await assertFromJavascript(page, `const items = [...document.querySelectorAll('.mantine-Accordion-item')];
const it = items.find(i => {
  const cc = i.querySelector('.mantine-Accordion-control');
  return cc && (cc.textContent || '').includes('DD SYNTHETIC MOBILE');
...
return true;`, 30000);
    // Let the accordion collapse
    await wait(page, 2);
    // RESTORED: no modal is open and the row is collapsed again
