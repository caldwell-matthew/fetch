...
export async function mob350(page: Page): Promise<void> {
  try {
    ...
    await el(page, `//*[@role="tab"][contains(normalize-space(.), "Equipment")]`).click({ timeout: DEFAULT_TIMEOUT });
    // Let the Equipment cards render
    await wait(page, 2);
    // BEFORE: count the equipment charge cards already here (the server-proof baseline)
...
return true;`, 30000);
    ...
    // Focus the equipment lookup
    await el(page, `//*[@id="equipmentId"]`).click({ timeout: DEFAULT_TIMEOUT });
    // Type into the equipment lookup to load options
    await el(page, `//*[@id="equipmentId"]`).fill(`AC Adapter`, { timeout: DEFAULT_TIMEOUT });
    // Wait for equipment options
    await wait(page, 2);
    // Pick the AC Adapter equipment option
    await el(page, `//*[@role="option"][contains(normalize-space(.), "AC Adapter")]`).click({ timeout: DEFAULT_TIMEOUT });
    // Enter a quantity
    ...
    await wait(page, 2);
    // ⭐ SERVER PROOF: after a RELOAD there is exactly ONE more equipment charge card than before
    await assertFromJavascript(page, `const tabEl = document.querySelector('[role="tab"][aria-selected="true"], [role="tab"][data-active]');
const byId = tabEl && tabEl.getAttribute('aria-controls')
  ? document.getElementById(tabEl.getAttribute('aria-controls')) : null;
const p = byId || [...document.querySelectorAll('[role="tabpanel"]')]
