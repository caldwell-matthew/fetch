// Converted on 2026-09-23 from the Datadog test legacy/Mobile/dd_tests_mobile/MOB.350_Work_Add_Equipment_Charge.json. This file is the source now: edit it directly.
...
export async function mob350(page: Page): Promise<void> {
...
return Date.now() - since >= 10000;`, 360000);
  ...
  await run.step("Open the Equipment tab", {}, async () => {
    await click(page, `//*[@role="tab"][contains(normalize-space(.), "Equipment")]`, DEFAULT_TIMEOUT);
  });
  await run.step("Let the Equipment cards render", {}, async () => {
    await wait(page, 2);
  });
  await run.step("BEFORE: count the equipment charge cards already here (the server-proof baseline)", {}, async () => {
...
if (!p) return false;
const needles = ['AC Adapter', 'Charged created on'];
...
return true;`, 30000);
  ...
  await run.step("Focus the equipment lookup", {}, async () => {
    await click(page, `//*[@id="equipmentId"]`, DEFAULT_TIMEOUT);
  });
  await run.step("Type into the equipment lookup to load options", {}, async () => {
    await typeText(page, `//*[@id="equipmentId"]`, `AC Adapter`, DEFAULT_TIMEOUT);
  });
  await run.step("Wait for equipment options", {}, async () => {
    await wait(page, 2);
  });
  await run.step("Pick the AC Adapter equipment option", {}, async () => {
    await click(page, `//*[@role="option"][contains(normalize-space(.), "AC Adapter")]`, DEFAULT_TIMEOUT);
  });
  ...
  await run.step("\u2b50 SERVER PROOF: after a RELOAD there is exactly ONE more equipment charge card than before", {}, async () => {
    await assertFromJavascript(page, `const tabEl = document.querySelector('[role="tab"][aria-selected="true"], [role="tab"][data-active]');
const byId = tabEl && tabEl.getAttribute('aria-controls')
  ? document.getElementById(tabEl.getAttribute('aria-controls')) : null;
const p = byId || [...document.querySelectorAll('[role="tabpanel"]')]
