// Converted on 2026-09-23 from the Datadog test legacy/Mobile/dd_tests_mobile/MOB.380_Work_Add_Other_Charge.json. This file is the source now: edit it directly.
...
return Date.now() - since >= 10000;`, 360000);
  ...
  await run.step("Open the Other tab", {}, async () => {
    await click(page, `//*[@role="tab"][contains(normalize-space(.), "Other")]`, DEFAULT_TIMEOUT);
  });
  await run.step("Let the Other cards render", {}, async () => {
    await wait(page, 2);
  });
  await run.step("BEFORE: count the other charge cards already here (the server-proof baseline)", {}, async () => {
...
return true;`, 30000);
  ...
  await run.step("Focus the other charge type lookup", {}, async () => {
    await click(page, `//*[@id="otherChargeId"]`, DEFAULT_TIMEOUT);
  });
  ...
  await run.step("Enter a unit price", {}, async () => {
  ...
  await run.step("\u2b50 SERVER PROOF: after a RELOAD there is exactly ONE more other charge card than before", {}, async () => {
    await assertFromJavascript(page, `const tabEl = document.querySelector('[role="tab"][aria-selected="true"], [role="tab"][data-active]');
const byId = tabEl && tabEl.getAttribute('aria-controls')
  ? document.getElementById(tabEl.getAttribute('aria-controls')) : null;
const p = byId || [...document.querySelectorAll('[role="tabpanel"]')]
