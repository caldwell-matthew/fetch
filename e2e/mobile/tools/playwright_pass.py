"""Run the converted suites under Playwright, in the schedule's safe order — 0 Datadog runs.
    ...
    .venv/bin/python e2e/mobile/tools/playwright_pass.py --stage 1        # the read-only suites
    .venv/bin/python e2e/mobile/tools/playwright_pass.py --stage 2        # the data-changing ones, one at a time
    .venv/bin/python e2e/mobile/tools/playwright_pass.py --from MOB.963   # resume stage 2 at a suite
...
WHY THIS EXISTS AND `npx playwright test` IS NOT ENOUGH
  ...
  Stage 2  the DATA-CHANGING suites, ONE AT A TIME, in `order` (tools/suites.json) — Session (MOB.973)
           last. The fixture checks (`tools/fixtures.py`) run before each one: a fixture that
...
The summary is printed and saved to `e2e/results/passes/mobile/<time>-playwright.md`, and each suite's
...
def spec_name(s):
    return os.path.basename(s["spec"])[:-len(".spec.ts")]


def plan():
    """(read-only suite names, data-changing names in pass order), skipping held suites."""
    run = [s for s in SUITES if "held" not in s]
    read_only = [spec_name(s) for s in run if not s["writes"]]
    writes = [spec_name(s) for s in sorted((s for s in run if s["writes"]), key=lambda s: s["order"])]
    return read_only, writes


def fixtures_at_rest():
    return subprocess.run([sys.executable, "fixtures.py", *FIXTURE_CHECKS], cwd=HERE).returncode == 0
