 ...
 * the REAL component source, on the bench, before spending a test run on them.
 *
 *     node e2e/mobile/tools/check_js_assertions.js          (jsdom: `npm install` in e2e/)
 *
 * WHY THIS EXISTS
 *   A jsassert body is the only part of a test that cannot be checked until it runs, and a
 ...
 *   1. ⭐ THE CODE UNDER TEST IS EXTRACTED FROM THE TEST SOURCE (e2e/mobile/tests/*.ts), never
 *      retyped here. A harness that tests a retyped copy proves nothing about what ships
 *      (trap 19), and a passing bench with a red run is worse than no bench at all.
 ...
 *
 * READING THE TESTS. Each case names its test the Datadog-era way (`MOB.351_..._Estimates.json`);
 * that maps to `e2e/mobile/tests/MOB.351_..._Estimates.ts`. A step there is
 *     await run.step("<name>", {…}, async () => { await assertFromJavascript(page, `CODE`, T); });
 * and CODE is a template literal, so it is COOKED here exactly as JavaScript would (`\\` -> `\`,
 * `` \` `` -> `` ` ``, `\${` -> `${`, ...) - the string the browser's `new Function` receives.
 * A `${VAR}` interpolation in a JS body cannot be reproduced statically and is an error.
 * A case whose test is not in e2e/mobile/tests is REPORTED at the end and fails the run; it is
...
const MOBILE = path.dirname(__dirname);                 // e2e/mobile
const TESTS = path.join(MOBILE, 'tests');
...
if (!JSDOM) {
	...
	console.error('jsdom not found - VERIFIED NOTHING. Run `npm install` in e2e/ (it is a devDependency there).');
	process.exit(2);
}
...
/* ------------------------------------------------------------------ reading the TypeScript tests */

/** `MOB.351_X.json` (the old name) or `MOB.351_X` or `MOB.351_X.ts` -> the .ts file name. */
const tsName = (testFile) => testFile.replace(/\.(json|ts)$/, '') + '.ts';

/**
 * Read one JS string literal ('…', "…" or `…`) starting at src[i]; return [cooked value, index after].
 * A template's `${expr}` is kept as `{{ expr }}` and flagged, so a caller can refuse it.
 */
function readLiteral(src, i) {
	const q = src[i];
	if (q !== '"' && q !== "'" && q !== '`') throw new Error(`expected a string literal at ${i}: ${src.slice(i, i + 40)}`);
	let out = '', interp = false;
	for (i++; i < src.length; i++) {
		const c = src[i];
		if (c === q) return [out, i + 1, interp];
		if (c === '\\') {
			const n = src[++i];
			const simple = { n: '\n', t: '\t', r: '\r', b: '\b', f: '\f', v: '\v' };
			if (n in simple) out += simple[n];
			else if (n === '0' && !/[0-9]/.test(src[i + 1])) out += '\0';
			else if (n === 'x') { out += String.fromCharCode(parseInt(src.substr(i + 1, 2), 16)); i += 2; }
			else if (n === 'u' && src[i + 1] === '{') { const e = src.indexOf('}', i); out += String.fromCodePoint(parseInt(src.slice(i + 2, e), 16)); i = e; }
			else if (n === 'u') { out += String.fromCharCode(parseInt(src.substr(i + 1, 4), 16)); i += 4; }
			else if (n === '\r') { if (src[i + 1] === '\n') i++; }      // line continuation
			else if (n === '\n' || n === ' ' || n === ' ') { /* line continuation */ }
		...
		if (q === '`' && c === '$' && src[i + 1] === '{') {
			...
			out += `{{ ${src.slice(i + 2, j - 1).trim()} }}`;
			interp = true;
			i = j - 1;
			continue;
		}
		if (q === '`' && c === '\r') { if (src[i + 1] === '\n') i++; out += '\n'; continue; }   // template CRLF -> LF
		out += c;
	}
	throw new Error('unterminated string literal');
}

// The step helpers whose arguments carry strings, and which of those arguments are strings.
const HELPER = /\bawait\s+(assertFromJavascript|assertPageContains|assertPageLacks|assertElementContent|assertElementPresent|click|typeText|uploadStandIn)\s*\(\s*page\s*,\s*/g;
const STEP = /\bawait\s+run\.step\(\s*/g;

/**
 * A test's steps, parsed from its .ts, in the shape the cases below were written against (the old
 * Datadog JSON): `{ details: { steps: [{ name, type, params: { code | value | files } }] } }`.
 * Only the kinds the cases read are filled in: assertFromJavascript (`params.code`) and
 * uploadFiles (`params.files[].name`, from uploadStandIn).
 */
const parsed = new Map();
function loadTest(testFile) {
	const file = tsName(testFile);
	...
	const steps = starts.map((at, k) => {
		...
		const chunk = src.slice(afterName, k + 1 < starts.length ? starts[k + 1] : src.length);
		const step = { name, type: null, params: {} };
		const h = new RegExp(HELPER.source, 'g').exec(chunk);
		if (!h) return step;
		const argAt = h.index + h[0].length;
		if (h[1] === 'assertFromJavascript') {
			const [code, , interp] = readLiteral(chunk, argAt);
			step.type = 'assertFromJavascript';
			step.params.code = code;
			step.interpolated = interp;
		} else if (h[1] === 'uploadStandIn') {
			const [, afterXpath] = readLiteral(chunk, argAt);
			const open = chunk.indexOf('[', afterXpath), close = chunk.indexOf(']', open);
			const names = [];
			for (let j = open + 1; j < close;) {
				if (/["'`]/.test(chunk[j])) { const [v, e] = readLiteral(chunk, j); names.push(v); j = e; } else j++;
			}
			step.type = 'uploadFiles';
			step.params.files = names.map(name => ({ name }));
		} else {
			step.type = h[1];
		}
		return step;
...
function testExists(testFile) {
	if (fs.existsSync(path.join(TESTS, tsName(testFile)))) return true;
	notPorted.add(tsName(testFile));
	return false;
}

/** Pull a step's JS body out of the test source, by a fragment of its step name. */
function bodyOf(testFile, nameFragment) {
	const s = loadTest(testFile).details.steps.find(
		st => st.type === 'assertFromJavascript' && st.name.includes(nameFragment));
	if (!s) throw new Error(`${tsName(testFile)}: no assertFromJavascript step matching `
		+ `"${nameFragment}" - the step was renamed and this check went stale`);
	if (s.interpolated) throw new Error(`${tsName(testFile)}: "${s.name}" interpolates \${…} into its JS body - `
		+ 'the bench cannot know the runtime value');
 ...
 * ========================================================================================= */
if (!testExists('MOB.386_Work_Condition_Edit_Save.json')) {
	console.log('\nMOB.386 - SKIPPED: MOB.386_Work_Condition_Edit_Save.ts is not in e2e/mobile/tests');
} else {
 ...
 * ========================================================================================= */
{
	...
	const stepsOf = (f) => loadTest(f).details.steps.filter(s => s.type === 'assertFromJavascript');
 ...
 * ========================================================================================= */
if (!testExists('MOB.712_AssetLookup_System_Create.json')
	|| !testExists('MOB.722_AssetLookup_Reading_Capture.json')) {
	console.log('\nMOB.712/722 - SKIPPED: a test is not in e2e/mobile/tests');
} else {
 ...
 * ========================================================================================= */
if (!testExists('MOB.866_MaterialLookup_Item_Attachment_Delete.json')) {
	console.log('\nMOB.866 - SKIPPED: a test is not in e2e/mobile/tests');
} else {
...
if (!['MOB.363_Work_Attachment_Upload_Delete.json', 'MOB.364_Work_Attach_Form.json', 'MOB.365_Work_Reassign_Stage.json']
	.map(f => testExists(f)).every(Boolean)) {
	console.log('\nMOB.363/364/365 - SKIPPED: a test is not in e2e/mobile/tests');
} else {
...
Promise.all(pending).then(() => {
	// A case whose test is gone was skipped above; that is a hole in the bench, so it fails the run.
	if (notPorted.size) {
		console.log(`\nNOT PORTED - cases skipped because their test is not in e2e/mobile/tests: ${[...notPorted].join(', ')}`);
	}
	const bad = failures || notPorted.size;
	console.log(failures ? `\n${failures} FAILURE(S)` : notPorted.size ? '\nNO FAILURES, BUT CASES WERE SKIPPED (above)' : '\nALL PASS');
