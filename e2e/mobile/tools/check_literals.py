...
WHY THIS EXISTS - the failure `check_drift` and `audit_assertions` structurally cannot see.
  Those two watched the TESTS: did the generator still reproduce its JSON, does an assertion
...
READING THE TESTS - the Playwright TypeScript in e2e/mobile/tests/*.ts plus the shared login
  (e2e/mobile/support/login.ts). Each step is `await run.step("<name>", {...}, async () => {
  await <helper>(page, `...`, ...); })`; the helper's string arguments are template literals and
  are COOKED here exactly as JavaScript would (a doubled backslash -> one, backslash-backtick ->
  a backtick, backslash-dollar-brace -> `${`).
  A real `${RUNID}` interpolation is read back as `{{ RUNID }}` - the Datadog-era spelling - so
  TEMPLATE strips it and the allowlist's `{{ RUNID }}` entries still match. Helpers read:
    assertPageContains / assertPageLacks (text), assertElementContent (xpath, text),
    assertFromJavascript (code), and the xpath of click / typeText / assertElementPresent /
    uploadStandIn. A typed VALUE is not an assertion and is not read (nor was it before).
  The login steps are read ONCE, from login.ts; under Datadog they were copied into every suite.

SELF-TEST
    ./.venv/bin/python e2e/mobile/tools/check_literals.py --self-test
...
USAGE
    ./.venv/bin/python e2e/mobile/tools/check_literals.py
    ./.venv/bin/python e2e/mobile/tools/check_literals.py --ref origin/development
    ./.venv/bin/python e2e/mobile/tools/check_literals.py --all   # show COMPOSED too
...
MOBILE = os.path.dirname(HERE)                          # e2e/mobile
...
# Templating - `{{ RUNID }}` (read from a TS `${RUNID}`, see READING THE TESTS) is substituted at run time and is never in source.
...
# ---------------------------------------------------------------------------- reading the .ts tests
_SIMPLE_ESCAPES = {"n": "\n", "t": "\t", "r": "\r", "b": "\b", "f": "\f", "v": "\v"}


def read_literal(src, i):
    """Read the JS string literal ('..', ".." or `..`) at src[i]; return (cooked value, index after).

    Cooked exactly as JavaScript would, so the value is what the test hands the browser. A
    template's `${expr}` comes back as `{{ expr }}` - its runtime value is unknowable here.
    """
    q = src[i]
    if q not in "'\"`":
        raise ValueError(f"expected a string literal at {i}: {src[i:i + 40]!r}")
    out, i = [], i + 1
    while i < len(src):
        c = src[i]
        if c == q:
            v = "".join(out)
            # "\ud83d\udcca" decodes to two surrogates; join them into the one character
            return v.encode("utf-16", "surrogatepass").decode("utf-16"), i + 1
        if c == "\\":
            i += 1
            n = src[i]
            if n in _SIMPLE_ESCAPES:
                out.append(_SIMPLE_ESCAPES[n])
            elif n == "0" and not src[i + 1:i + 2].isdigit():
                out.append("\0")
            elif n == "x":
                out.append(chr(int(src[i + 1:i + 3], 16))); i += 2
            elif n == "u" and src[i + 1:i + 2] == "{":
                e = src.index("}", i); out.append(chr(int(src[i + 2:e], 16))); i = e
            elif n == "u":
                out.append(chr(int(src[i + 1:i + 5], 16))); i += 4
            elif n == "\r":
                if src[i + 1:i + 2] == "\n":
        ...
        if q == "`" and c == "$" and src[i + 1:i + 2] == "{":
            ...
            while j < len(src) and depth:
                ...
                j += 1
            out.append("{{ " + src[i + 2:j - 1].strip() + " }}")
            i = j
            continue
        if q == "`" and c == "\r":
            if src[i + 1:i + 2] == "\n":
                i += 1
            c = "\n"                               # a template normalises CRLF to LF
        out.append(c)
        i += 1
    raise ValueError("unterminated string literal")


STEP = re.compile(r"\bawait\s+run\.step\(\s*")
# helper -> which of its arguments after `page` are strings, by role
HELPERS = {
    "assertPageContains": ("value",), "assertPageLacks": ("value",),
    "assertElementContent": ("xpath", "value"), "assertFromJavascript": ("code",),
    "assertElementPresent": ("xpath",), "click": ("xpath",), "typeText": ("xpath", "typed"),
    "uploadStandIn": ("xpath",),
}
HELPER = re.compile(r"\bawait\s+(" + "|".join(HELPERS) + r")\s*\(\s*page\s*,\s*")


def ts_steps(path):
    """Yield (line, step_name, helper, {role: string}) for every `run.step` in a .ts test."""
    src = open(path, encoding="utf-8").read()
    starts = [m.end() for m in STEP.finditer(src)]
    for k, at in enumerate(starts):
        name, after = read_literal(src, at)
        end = starts[k + 1] if k + 1 < len(starts) else len(src)
        chunk = src[after:end]
        line = src.count("\n", 0, at) + 1
        h = HELPER.search(chunk)
        if not h:
            yield line, name, None, {}
            continue
        args, j = {}, h.end()
        for role in HELPERS[h.group(1)]:
            while chunk[j] in " \t\r\n,":
                j += 1
            if chunk[j] not in "'\"`":
                break                               # a non-literal argument: stop reading
            args[role], j = read_literal(chunk, j)
        yield line, name, h.group(1), args


def test_files():
    """(display name, path) for every test, plus the shared login."""
    out = [(os.path.basename(p)[:-3], p) for p in sorted(glob.glob(os.path.join(TESTS, "*.ts")))]
    return out + [("support/login", LOGIN)]


def literals():
    """Yield (test, line, step_name, kind, literal) for everything asserted.

    The same kinds the Datadog-era reader took from the JSON: page-contains/lacks values,
    element-content values, text and attribute predicates in ANY step's xpath, and prose in JS
    bodies. `line` is the step's line in the .ts (the JSON reader gave a step index).
    """
    for test, path in test_files():
        for line, sname, helper, a in ts_steps(path):
            if helper in ("assertPageContains", "assertPageLacks", "assertElementContent") \
                    and isinstance(a.get("value"), str):
                yield test, line, sname, helper, a["value"]
            xp = a.get("xpath") or ""
            for m in XPATH_TEXT.finditer(xp):
                got = next((g for g in m.groups() if g), None)
                if got:
                    yield test, line, sname, "xpath", got
            for m in XPATH_ATTR.finditer(xp):
                yield test, line, sname, "attr", m.group(1)
            if helper == "assertFromJavascript":
                for lit in js_prose(a.get("code") or ""):
                    yield test, line, sname, "js", lit
...
def main():
    ...
    for test, i, sname, kind, lit in literals():
        raw = lit.strip()
        # strip templating before matching - `{{ RUNID }}` is never in source
