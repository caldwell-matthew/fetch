...
| Tests | **145 leaf tests · 24 suites** · 5097 steps · 139 subtest slots (+ `MOB.999_Verify_Scratch`, a harness; `MOB.978_DIAG_WorkList_Probe` and `MOB.977_DIAG_Condition_Form_Schema_Race`, diagnostics; `MOB.PDF_Upload_Recording`, which exists only on Datadog — never delete it) |
| Local ↔ remote | ⚠️ **Local is ahead of Datadog, deliberately.** Datadog is frozen (▶ #37), and since 2026-09-23 29 tests have changed locally: the work-list idle gate in the 27 that use it, `MOB.345`'s narrowing, `MOB.820`'s fixed-bug assertions. `preflight.py sync` reports them; it stays red until the tests are pushed or Datadog is retired. `MOB.711` is archived locally only |
...
| # | item | state |
|---|---|---|
| **37** | **Run the suites automatically — moving from Datadog to Playwright + CircleCI** (owner, 2026-09-22). ⛔ Datadog is paused by the owner's manager (2026-09-18) after Parallel Testing Slots billed $513 in a month with the concurrency cap at 10; the cap is back to 1 and all 433 tests are paused. **Where it stands:** every suite is converted to Playwright (TypeScript, `e2e/`) by `to_playwright.py`, faithfully — every `wait` kept — and `check_conversion.py` confirms each of the 4,486 steps and suite children matches its JSON. **All 23 scheduled suites pass locally against dev** (the 10 read-only 60/60; the 13 data-changing ones one at a time with the fixture checks between them, via `playwright_pass.py`), and the fixtures read back at rest. `MOB.967` is held: its `MOB.600` photo-picker step was recorded without an xpath and cannot run outside Datadog. Getting there fixed two pre-existing test faults under BOTH runners: the shared work-list gate passed in the gaps between the list's six loading phases (now: no loading bar for 10s straight — `MOB.350`'s equipment picker had an empty cache), and `MOB.345` narrowed to a set that kept growing (now the two fixture stages named `USED IN DATADOG`). **Left:** the CircleCI job (draft at `e2e/ci/circleci-e2e.yml`; needs: can CircleCI reach dev, which context holds the login, where results go); block Datadog RUM inside the tests so their sessions are not billed as users; a `fixtures.ts` for record names, ids and `DD SYNTHETIC` markers; tighten the 1,179 fixed waits suite by suite, measured either side; move `e2e/` into MentorTwo; decide when Datadog is switched off for good (its 433 tests and 250 global variables are backed up in `legacy/dd_tests_backup/`) | in progress |
